...
def create_metric_plot(df, depots, parts_unique, metric, title, by_depot=True, figsize=(8, 4), max_legend_entries=15):
    """
    ...
    Args:
        ...
        max_legend_entries: Maximum number of legend entries before using colorbar
    ...
    if by_depot:
        use_colorbar = len(depots) > max_legend_entries
        for i, depot in enumerate(depots):
            depot_df = df[df['depot_capacity'] == depot]
            means = depot_df.groupby('n_total_parts')[metric].mean()
            ax.plot(means.index, means.values, 'o-', 
                    label=f'{depot}' if not use_colorbar else None, 
                    color=colors_by_depot[i], markersize=4, linewidth=1.5)
        ax.set_xlabel('N Total Parts', fontsize=10)
        if use_colorbar:
            sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(vmin=min(depots), vmax=max(depots)))
            sm.set_array([])
            cbar = fig.colorbar(sm, ax=ax, pad=0.02)
            cbar.set_label('Depot Cap', fontsize=9)
        else:
            ax.legend(title='Depot Cap', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=8)
    else:
        use_colorbar = len(parts_unique) > max_legend_entries
        for i, n_parts in enumerate(parts_unique):
            parts_df = df[df['n_total_parts'] == n_parts]
            means = parts_df.groupby('depot_capacity')[metric].mean()
            ax.plot(means.index, means.values, 'o-', 
                    label=f'{int(n_parts)}' if not use_colorbar else None, 
                    color=colors_by_parts[i], markersize=4, linewidth=1.5)
        ax.set_xlabel('Depot Capacity', fontsize=10)
        if use_colorbar:
            sm = plt.cm.ScalarMappable(cmap='plasma', norm=plt.Normalize(vmin=min(parts_unique), vmax=max(parts_unique)))
            sm.set_array([])
            cbar = fig.colorbar(sm, ax=ax, pad=0.02)
            cbar.set_label('N Parts', fontsize=9)
        else:
