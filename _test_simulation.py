...
use_calculated_allocation = True  # Set to True to use calculated allocation, False for hardcoded
...
# Create SimulationEngine with Parameters object (creates its own DataSets internally)
...
# Get final DataFrames from sim_engine.datasets
datasets = sim_engine.datasets
...
# Calculate average MICAP count from aircraft WIP snapshots
wip_ac_df = datasets.wip_ac_df

# Average including zero values
micap_all = wip_ac_df['micap']
average_micap_with_zeros = micap_all.mean()
count_all_observations = len(micap_all)

# Average excluding zero values
micap_nonzero = wip_ac_df[wip_ac_df['micap'] > 0]['micap']
...
func9_result = function_nine(all_parts_df, condemn_cycle=params['condemn_cycle'])
...
# EXPORT TO EXCEL
# ============================================================================
# Export DataFrames to Excel with separate sheets
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    datasets.wip_df.to_excel(writer, sheet_name='wip_df', index=False)
    datasets.wip_ac_df.to_excel(writer, sheet_name='wip_ac_df', index=False)
